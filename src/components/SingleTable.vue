<template>
    <div class="hera">
        ...
        >
            ...
            <!-- 如果数据还没请求出来会旋转 -->
            <div class="loadingnamesTo"  v-show="thesearchId != searchId">
                <van-loading
                    color="#0094ff"
                    size="80px"
                    class="loadingnames"
            ...
            <div class="ssd" ref="scrol" @scroll="scs($event)">
                ...
                <!--  -->
                <!-- {{ axoData }} -->
                <div class="main" v-show="thesearchId == searchId">
                    <div
                        class="nav-img"
                        :style="{
                            'background-image': `url(${axoData.artist?.img1v1Url})`,
                        }"
                    >
                ...
                <!-- 名字和头像和喜欢和分析 -->
                <div class="main-seco" v-show="thesearchId == searchId">
                    <div class="seco-img">
                        <img
                            :src="axoData.artist?.picUrl"
                            alt=""
                            v-lazy="axoData.artist?.picUrl"
                        />
                    ...
                    <div class="seco-icon">
                        ...
                        <div class="aixin" @click="zhuanfaGao">
                            <div
                                class="aixin-icon"
                                :class="{ col2: zhuanfanum == searchId }"
                            >
                                <van-icon name="share" />
                            </div>
                ...
                <!-- 音乐播放的全部歌曲 -->
                <div v-show="thesearchId == searchId">
                    <MusicTable
...
import MusicTable from "./SingleTable/MusicTable.vue";
...
export default {
    ...
    methods: {
        ...mapMutations(["searchShowend", "searchIdend"]),

        // 点击爱心高亮
        aixinGao() {
            // linkMath没有this.searchId时push进去
            if (this.aixinMath.indexOf(this.searchId) == -1) {
                this.aixinMath.push(this.searchId);
                localStorage.setItem("aixinMath", JSON.stringify(this.aixinMath));
                // 如果this.aixin有万字就不添加
                this.aixin = this.aixin + 1;

                this.aiLove = this.searchId;
            } else {
                this.aixinMath.splice(this.aixinMath.indexOf(this.searchId), 1);
                localStorage.setItem("aixinMath", JSON.stringify(this.aixinMath));
                this.aixin = this.aixin - 1;
                this.aiLove = '';
            }
        ...
        zhuanfaGao() {
            // linkMath没有this.searchId时push进去
            if (this.zhuanfaMath.indexOf(this.searchId) == -1) {
                this.zhuanfaMath.push(this.searchId);
                ...
                this.zhuanfanum = this.searchId;
            } else {
                this.zhuanfaMath.splice(
                    this.zhuanfaMath.indexOf(this.searchId),
                    1
                );
                localStorage.setItem(
                    "zhuanfaMath",
        ...
        ddsaxios(ids) {
            ...
            this.$axios
                ...
                .then((res) => {
                    ...
                    this.axoDatamusic = res.songs;
                    console.log("this.axoDatamusic", this.axoDatamusic);
                    this.axoData = res.album;
                    console.log(this.axoData);
                    // // return res;
                    this.thesearchId = ids;
                    this.img = res.album.blurPicUrl;
                    console.log(res.album.blurPicUrl);
                });
            ...
            this.$axios
                ...
                .then((res) => {
                    ...
                    this.aixin = this.aixin + res.commentCount;

                    this.zhuanfa = this.zhuanfa + res.shareCount;
                });
        },
    },
    watch: {
